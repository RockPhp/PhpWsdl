...
// static classes workaround to php 5.1.6

require_once(dirname(__FILE__).'/class.phpwsdl.static.php');
require_once(dirname(__FILE__).'/class.phpwsdlformatter.static.php');
require_once(dirname(__FILE__).'/class.phpwsdlobject.static.php');
require_once(dirname(__FILE__).'/class.phpwsdlparser.static.php');
require_once(dirname(__FILE__).'/class.phpwsdlproxy.static.php');
require_once(dirname(__FILE__).'/class.phpwsdlparam.static.php');
require_once(dirname(__FILE__).'/class.phpwsdlmethod.static.php');
require_once(dirname(__FILE__).'/class.phpwsdlelement.static.php');
require_once(dirname(__FILE__).'/class.phpwsdlcomplex.static.php');
...
class PhpWsdl{
	...
	public static function EnableCache($folder=null,$timeout=null){
		if(is_null($folder)){
			...
			}else if(self::IsCacheFolderWriteAble('/tmp')){
				$folder='/tmp';
			}else{
...
// 		$input=fopen('data://text/plain,'.$xml,'r');
		...
		$fiveMBs = 5 * 1024 * 1024;
		$input = fopen("php://temp/maxmemory:$fiveMBs", 'r+');
		fputs($input, $xml);
		rewind($input);
	...
	public static function CallHook($name,$data=null){
		...
		while(++$i<$len){
			...
			$strCall = self::$Config['extensions'][$name][$keys[$i]];
			$arrayCall = explode('::',$strCall);
			$className = $arrayCall[0].'Static';
			$methodName = $arrayCall[1];
			$obj = new $className();
// 			echo '<textarea>';
